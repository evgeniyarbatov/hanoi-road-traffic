...
def calculate_complex_importance_score(df):
    ...
    if "total_intersections" in df.columns and "major_intersections" in df.columns:
        ...
        total_int_norm = (
            df["total_intersections"] / df["total_intersections"].max() * 50
            if df["total_intersections"].max() > 0
            else 0
        )
        major_int_norm = (
            df["major_intersections"] / df["major_intersections"].max() * 50
            if df["major_intersections"].max() > 0
            else 0
        )
        df["intersection_score"] = total_int_norm + (
            major_int_norm * 2
        )  # Weight major intersections more
    elif "total_intersections" in df.columns:
        df["intersection_score"] = (
            df["total_intersections"] / df["total_intersections"].max() * 100
            if df["total_intersections"].max() > 0
            else 0
        )
    ...
    if "avg_node_degree" in df.columns and "intersection_density" in df.columns:
        ...
        df["avg_node_degree_clean"] = pd.to_numeric(
            df["avg_node_degree"].astype(str).str.replace("**", ""), errors="coerce"
        )
        df["intersection_density_clean"] = pd.to_numeric(
            df["intersection_density"].astype(str).str.replace("**", ""),
            errors="coerce",
        )

        avg_degree_norm = (
            df["avg_node_degree_clean"] / df["avg_node_degree_clean"].max() * 50
            if df["avg_node_degree_clean"].max() > 0
            else 0
        )
        int_density_norm = (
            df["intersection_density_clean"]
            / df["intersection_density_clean"].max()
            * 50
            if df["intersection_density_clean"].max() > 0
            else 0
        )
        df["connectivity_score"] = avg_degree_norm + int_density_norm
    elif "max_node_degree" in df.columns:
        df["connectivity_score"] = (
            df["max_node_degree"] / df["max_node_degree"].max() * 100
            if df["max_node_degree"].max() > 0
            else 0
        )
    ...
    if "lanes" in df.columns:
        ...
        traffic_score += (
            (lanes_clean / lanes_clean.max() * 30) if lanes_clean.max() > 0 else 0
        )

    if "maxspeed" in df.columns:
        speed_clean = pd.to_numeric(df["maxspeed"], errors="coerce").fillna(
            30
        )  # Default 30 km/h
        traffic_score += (
            (speed_clean / speed_clean.max() * 30) if speed_clean.max() > 0 else 0
        )

    if "running_score" in df.columns:
        running_norm = (
            df["running_score"] / df["running_score"].max() * 40
            if df["running_score"].max() > 0
            else 0
        )
...
def create_plotly_map(road_segments_df, map_file):
    ...
    fig.update_layout(
        mapbox=dict(
            center=dict(
                lat=road_segments_df["lat"].mean(), lon=road_segments_df["lon"].mean()
            )
        ),
        height=700,
        title_x=0.5,
...
Examples:
  ...
  python road_analysis.py data.csv --output-prefix hanoi_road_segments
    ...
    print(
        f"Top {len(road_segments_df)} most important road segments analyzed (≤{args.max_distance}m)"
    )
