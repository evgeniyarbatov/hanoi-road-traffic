...
def save_to_postgis(data, lat, lon, timestamp=None):
    ...
    try:
        coord_list = (
            data.get("flowSegmentData", {}).get("coordinates", {}).get("coordinate", [])
        )
        if not coord_list:
        ...
        linestring = LineString(
            [(pt["longitude"], pt["latitude"]) for pt in coord_list]
        )
        ...
        if timestamp:
            ...
            """,
                (lat, lon, json.dumps(data), lon, lat, linestring.wkt, timestamp),
            )
        else:
            cur.execute(
                """
            ...
            """,
                (lat, lon, json.dumps(data), lon, lat, linestring.wkt),
            )
...
def collect_for_24_hours_variable(coordinates):
    ...
    while datetime.now() < end_time:
        ...
        for i, (lat, lon) in enumerate(coordinates, 1):
            ...
            print(
                f"Processing coordinate {i} of {len(coordinates)} ({percent}%): {lat}, {lon}"
            )
        ...
        print(
            f"Collection #{collection_count} completed in {time.strftime('%H:%M:%S', time.gmtime(collection_duration))}"
        )
        ...
        if next_collection_time < end_time:
            ...
            if sleep_seconds > 0:
                print(
                    f"Waiting {sleep_seconds/60:.1f} minutes until next collection..."
                )
...
def preview_collection_schedule(num_coordinates=60):
    ...
    while current_time < end_time:
        ...
        collections.append(
            {"number": collection_count, "time": current_time, "interval": interval}
        )

        # Show first few and transition points
        if collection_count <= 5 or interval != get_collection_interval(
            current_time + timedelta(minutes=interval), num_coordinates
        ):
            print(
                f"Collection #{collection_count:2d}: {current_time.strftime('%H:%M:%S')} (next in {interval:4.1f} min)"
            )
    ...
    print(
        f"Standard resolution periods: {standard_collections} collections (every 75 min)"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect traffic data from TomTom API using CSV coordinates"
    )
    parser.add_argument(
        "csv_file", help="Path to CSV file containing lat,lon coordinates"
    )
    parser.add_argument(
        "--continuous", "-c", action="store_true", help="Run continuously for 24 hours"
    )
